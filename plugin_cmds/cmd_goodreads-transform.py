...
def _prepare_library_for_export(library):
    ...
    logger.debug("ISBNs from API: %d", isbn_api_counter)
    logger.debug("ISBNs requested with isbntools: %d", isbn_counter)
    logger.debug("No result with isbntools: %d", isbn_no_result_counter)
    logger.debug(
        "title skipped from file due to no ISBN or title not read: %d", skipped_items
    )
